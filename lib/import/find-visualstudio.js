...
class VisualStudioFinder {
	...
	getToolset(info, versionYear) {
		if (process.env?.XCRAFT_NODEGYP_WIN_TOOLSET) {
			return process.env.XCRAFT_NODEGYP_WIN_TOOLSET;
		}
	...
	getSDK(info) {
		if (process.env?.XCRAFT_NODEGYP_WIN_SDK) {
			return process.env.XCRAFT_NODEGYP_WIN_SDK;
		}
